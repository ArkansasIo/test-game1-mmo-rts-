...
try {
    switch ($action) {
        ...
        case 'mothership_upgrade': if(isset($_POST['moon_id'])){$upgrade=(new WorldService(db()))->upgradeMoonGate((int)$user['id'],(int)$_POST['moon_id']);$_SESSION['moon_gate_upgrade']=$upgrade;$_SESSION['flash']='Jump gate upgraded to level '.$upgrade['level_after'].' for '.number_format($upgrade['cost']).' Naquadah.';}else{$upgrade=(new MothershipService(db()))->upgrade((int)$user['id'],(string)$_POST['module']);$_SESSION['mothership_upgrade']=$upgrade;$_SESSION['flash']='Mothership '.$upgrade['module'].' queued to level '.$upgrade['level_after'].' for '.number_format($upgrade['cost']).' Naquadah.';} break;
        case 'combat':
        ...
        case 'coordinate_lookup': $_SESSION['coordinate_lookup']=(new WorldService(db()))->coordinateLookup((int)$user['id'],(string)$_POST['coordinate']); $_SESSION['flash']='Coordinate lookup completed.'; break;
