...
try {
    switch ($action) {
        ...
        case 'technology': $technologyKey=(string)$_POST['technology_key']; $categoryStmt=db()->prepare('SELECT category FROM technologies WHERE technology_key=?'); $categoryStmt->execute([$technologyKey]); $category=(string)$categoryStmt->fetchColumn(); if($category==='defense'){ $research=(new DefenseTechnologyService(db()))->upgrade((int)$user['id'],$technologyKey); $_SESSION['flash']='Defense research queued to level '.$research['level_after'].' for '.number_format($research['cost']).' Naquadah.'; } elseif($category==='offense'){ $research=(new OffenseTechnologyService(db()))->upgrade((int)$user['id'],$technologyKey); $_SESSION['flash']='Offense research queued to level '.$research['level_after'].' for '.number_format($research['cost']).' Naquadah.'; } elseif($category==='covert'){ $research=(new CovertTechnologyService(db()))->upgrade((int)$user['id'],$technologyKey); $_SESSION['flash']='Covert research queued to level '.$research['level_after'].' for '.number_format($research['cost']).' Naquadah.'; } else { $research=(new TechnologyTreeService(db()))->upgrade((int)$user['id'],$technologyKey); $_SESSION['flash']='Research queued to level '.$research['level_after'].' for '.number_format($research['cost']).' Naquadah.'; } break;
