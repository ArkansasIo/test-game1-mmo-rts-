...
function stargatewars_premium_premium_services_validate_intent(array $input): array {
    ...
    if ($action === '' || !in_array($action, stargatewars_premium_premium_services_actions(), true)) { $errors['action'] = 'Action is not permitted for this page.'; }
    if (in_array($action, ['combat','combat:raid','covert:recon','covert:spy','covert:sabotage'], true) && (int)($input['target_id'] ?? 0) <= 0) { $errors['target_id'] = 'A valid target is required.'; }
    if (in_array($action, ['deposit','withdraw','train','upgrade_up','technology','weapon_buy','weapon_repair'], true) && (int)($input['amount'] ?? $input['quantity'] ?? 0) < 0) { $errors['amount'] = 'The requested amount must not be negative.'; }
    return ['valid' => $errors === [], 'errors' => $errors, 'action' => $action];
}
function stargatewars_premium_premium_services_preview(array $context = []): array {
    return ['route' => 'premium-services', 'title' => 'Premium Services', 'logic' => stargatewars_premium_premium_services_logic(), 'features' => stargatewars_premium_premium_services_features(), 'design' => stargatewars_premium_premium_services_design(), 'systems' => stargatewars_premium_premium_services_systems(), 'context' => $context];
}
