...
function stargatewars_premium_store_validate_intent(array $input): array {
 $errors=[]; $action=(string)($input['action']??''); if($action===''||!in_array($action,stargatewars_premium_store_actions(),true)) $errors['action']='Action is not permitted for this Premium page.';
...
function stargatewars_premium_store_preview(array $context=[]): array { return ['route'=>'store','title'=>'Premium Store','logic'=>stargatewars_premium_store_logic(),'features'=>stargatewars_premium_store_features(),'design'=>stargatewars_premium_store_design(),'systems'=>stargatewars_premium_store_systems(),'context'=>$context]; }
